...
async function getWorker(): Promise<Worker> {
  ...
  try {
    ...
    await worker.setParameters({
      ...
      // 注意：之前这里有一行 tessedit_char_whitelist，末尾的字面量 '\u4e00-\u9fa5'
      // 不是 Unicode 区间（whitelist 不支持区间语法），会污染中文识别，已删除。
      // 中文识别完全交给 chi_sim 语言包，不再设白名单。
    })
...
export async function recognizePositionScreenshot(imageFile: File): Promise<OCRResult> {
  ...
  try {
    ...
    if (!tesseractResult.success || finalText.length < 50) {
      ...
      // 注意：之前这里有一段 hybrid 拼接逻辑——当本地和云端都有文本时直接拼接两份。
      // 这会导致同一只股票被解析两次，parsePositionData 把它当成两条持仓，数量翻倍。
      // 现在改为：云端结果只要更长就用云端，不拼接。
